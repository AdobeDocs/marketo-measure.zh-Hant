---
description: 適用於Marketo Measure使用者的Dynamics CRM指南使用 [!DNL Azure Active Directory] 的OAuth
...
1. 選擇Azure AD租使用者，方法是按一下頁面右上角的帳戶，然後按一下[切換目錄]導覽，然後選取適當的租使用者。 如果您的帳戶下只有一個Azure AD租使用者，或您已選取適當的Azure AD租使用者，請略過此步驟。
   ...
   ![1. 在](assets/bizible-taxonomy-1.png)中按一下您的帳戶以選擇Azure AD租使用者

1. 在搜尋列中搜尋「[!DNL Azure Active Directory]」，然後按一下名稱以開啟。

   ![1. 在搜尋列和](assets/microsoft-guide-1.png)中搜尋「Azure Active Directory」

1. 按一下左側功能表中的&#x200B;**[!UICONTROL App Registrations]**。

   ![1. 按一下左側功能表中的[應用程式註冊]。](assets/microsoft-guide-2.png)

1. 按一下頂端的&#x200B;**[!UICONTROL New Registration]**。

   ![1. 按一下頂端的[新增註冊]。](assets/microsoft-guide-3.png)
...
1. 按一下左側功能表中的&#x200B;**[!UICONTROL Authentication]**。
   ...
   ![1. 按一下左側功能表中的[驗證]。](assets/microsoft-guide-4.png)
...
1. 將[!DNL Marketo Measure]重新導向URL： `https://apps.bizible.com/OAuth2`與`https://apps.bizible.com/OAuth2?identityOnly=true`新增至重新導向URL清單。
   ...
   ![1. 將Marketo Measure重新導向URL： https://apps.bizible.com/OAuth2和https://apps.bizible.com/OAuth2?identityOnly=true新增至](assets/microsoft-guide-5.png)

1. 導覽至API許可權索引標籤，並確保為應用程式指派了正確的許可權。

   ![1. 瀏覽至API許可權索引標籤，並確定](assets/microsoft-guide-6.png)

1. 從這裡，在搜尋方塊中輸入&quot;[!UICONTROL enterprise]&quot;並按一下&#x200B;**[!UICONTROL Enterprise Applications]**。

   ![1. 從這裡，在搜尋方塊中輸入&quot;enterprise&quot;，然後按一下](assets/microsoft-guide-7.png)

1. 再次從應用程式清單中尋找並開啟您的新應用程式。

1. 在[許可權]索引標籤中，按一下&#x200B;**[!UICONTROL Grant Admin Consent for (instance name)]**。

   ![1. 在[許可權]索引標籤中，按一下[授與管理員同意] （執行個體](assets/microsoft-guide-8.png)）

1. 按一下「**[!UICONTROL Accept]**」。

   ![1. 按一下[接受]。](assets/microsoft-guide-9.png)

1. 從「[!UICONTROL Users and Groups]」索引標籤，確定已將有效的「使用者和群組」指派給應用程式。

   ![1. 在[使用者與群組]索引標籤中，確定](assets/microsoft-guide-10.png)
...
1. 第一次設定Dynamics連線時，請依照[本文章](/help/microsoft-dynamics-crm-installation-guide.md)中「CRM為資料提供者」章節的步驟1-5操作。
...
![b。使用者端密碼是在Azure入口網站](assets/microsoft-guide-11.png)中建立的應用程式密碼
...
1. 按一下金鑰時，會出現快顯視窗，提示您輸入使用者端ID、使用者端密碼和應用程式ID URI，類似於註冊流程。
   ...
   ![1. 按一下鍵時，會出現快顯視窗，而您是](assets/microsoft-guide-12.png)
