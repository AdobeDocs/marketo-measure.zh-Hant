...
![您需要將現有的連線資料更新到您的特定Snowflake](assets/marketo-tableau-7.png)

## 資料連線 {#data-connection}

您需要設定與Snowflake執行個體的資料連線。 為此，您需要伺服器名稱以及您的使用者名稱和密碼。 如需尋找此資訊及重設密碼的詳細資訊，請參閱[此處](/help/data-warehouse-access-reader-account.md){target="_blank"}。

![您必須設定與Snowflake執行個體的資料連線。](assets/marketo-tableau-5.png)
...
![您還需要輸入初始SQL命令。 這支援](assets/marketo-tableau-6.png)
...
![已對Tableau中的資料套用幾項轉換](assets/marketo-tableau-1.png)

![已對Tableau中的資料套用幾項轉換](assets/marketo-tableau-2.png)
...
![若要將貨幣轉換功能新增至模型中的計算，我們已](assets/marketo-tableau-4.png)
...
![有些地方有兩個來自Snowflake的資料表](assets/marketo-tableau-3.png)

### 區段名稱 {#segment-names}

由於區段名稱是可自訂的，因此在Snowflake Data Warehouse中會有通用的欄名稱。[!DNL BIZ_SEGMENT_NAMES] 為對應表格，如[!DNL Marketo Measure] UI中的區段區段所定義，此表格會列出一般區段名稱及其對應的自訂區段名稱。 如果您使用自訂區段名稱，並且想要更新您的[!DNL Tableau]模型以合併這些名稱，請使用此表格並手動重新命名Tableau模型內的欄。 區段欄位位於「銷售機會」和「歸因接觸點」表格中，且只需重新命名一次。
...
![名稱可以兩種方式更新。 第一個選項是](assets/marketo-tableau-14.png)

另一個選項是直接重新命名[!DNL Tableau]表格中的資料行。

![另一個選項是直接重新命名](assets/marketo-tableau-9.png)中的資料行

## 資料模型 {#data-model}

按一下下方影像以取得其完整大小版本。

[![按一下下方影像以取得完整大小版本。](assets/marketo-tableau-8.png)](assets/tableau-model-1.png){target="_blank"}
...
![成本和接觸點資料共用「管道」和「促銷活動」維度。 但是，Tableau](assets/marketo-tableau-10.png)
...
![將值從公司貨幣轉換為選取的公司貨幣值](assets/marketo-tableau-11.png)
...
![此模型中的貨幣轉換量值會取代1.0](assets/marketo-tableau-12.png)的值

## 資料定義 {#data-definitions}

已將引數、自訂欄和測量的定義新增至[!DNL Tableau model]。

![已將引數定義新增至Tableau模型，自訂](assets/marketo-tableau-15.png)

若要檢視直接來自[!DNL Snowflake]之資料行的定義，請參閱[資料倉儲檔案](/help/data-warehouse-schema.md){target="_blank"}。
