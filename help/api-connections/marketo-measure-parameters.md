...
若要進一步從UTM的使用取得insight，[!DNL Marketo Measure]會在[!DNL Google] AdWords、Bing Ads和[!DNL Facebook]廣告中將自訂引數附加至您的廣告。 [!DNL Marketo Measure]與這些平台整合，以自動化大部分的設定程式。 如果您選取使用自動標籤，[!DNL Marketo Measure]會自動將其引數附加至廣告的URL。 [!DNL Marketo Measure]也會自動從平台下載行銷成本，並將其載入至[!DNL Marketo Measure]應用程式。
