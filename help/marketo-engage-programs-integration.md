...
1. 若要開始在Marketo程式中設定規則，請瀏覽至&#x200B;**[!UICONTROL My Account]** > **[!UICONTROL Settings]** > **[!UICONTROL Programs]**。 按一下&#x200B;**+**&#x200B;圖示以開始建立您的第一個規則。
   ...
   ![1. 若要開始在Marketo程式中設定規則，請導覽至](assets/marketo-engage-programs-01.png)

   ![1. 若要開始在Marketo程式中設定規則，請導覽至](assets/marketo-engage-programs-06.png)

1. 如果有助於追蹤規則，您可以選擇設定規則名稱。 您首先會從計畫和計畫成員資格欄位清單中選取要定義規則的欄位。 選取要檢查的運運算元和預期值，以繼續建立規則。

   ![1. 您可以選擇為規則設定名稱，如果是](assets/marketo-engage-programs-07.png)

1. 在同一方塊內新增另一個陳述式，以在規則中設定「and」條件，或按一下方塊外的+圖示以設定「or」陳述式。

   ![1. 在同一方塊中新增另一個陳述式以設定](assets/bizible-discover-1.png)

1. 選擇用來對應至接觸點日期的日期或日期/時間欄位。 若要檢視Marketo中可用的值清單，請輸入花括弧`{`，我們就會顯示可用的欄位。

   ![1. 選擇應該使用哪個日期或日期/時間欄位來對應](assets/marketo-engage-programs-02.png)

   >[!NOTE]
   >
   >如果您的規則想要擷取活動日期，或方案成員達到特定狀態的日期，您將需要利用「[!DNL Marketo Engage]活動整合」，並為「進度變更狀態」活動型別設定規則。

   ![如果您的規則想要擷取活動日期或日期](assets/bizible-discover-2.png)
...
1. 若要執行測試，請按一下如下所示的&#x200B;**[!UICONTROL TEST]**&#x200B;按鈕。
   ...
   ![1. 若要執行測試，請按一下顯示的[測試]按鈕](assets/marketo-engage-programs-03.png)

1. 強制回應視窗會出現，您可在其中輸入來自Marketo的方案ID。

   ![1. 將會出現一個強制回應視窗，您可以在其中輸入程式](assets/marketo-engage-programs-04.png)
   ...
   ![規則是在5000個成員的樣本大小上執行。 若您的](assets/marketo-engage-programs-05.png)

   您可以按一下「成員計數」，檢視符合方案資格的Marketo人員ID清單。

   ![您可以按一下「成員計數」來檢視Marketo清單](assets/marketo-engage-activities-08.png)
...
1. 首先，選取應對應至值的管道，然後選擇性選取子管道。 完成後，請按一下底部的&#x200B;**[!UICONTROL Save]**。
   ...
   ![1. 請先選取應該對應到值的頻道，然後](assets/marketo-engage-programs-08.png)
...
`mark@email.com`會收到電子郵件，然後點進並登陸wayneenterprises.com。 這會變成使用相同Cookie Id對`mark@email.com's`進行`wayneenterprise.com`次的第二次造訪，但沒有任何表單填入，因此對於[!DNL Marketo Measure]，他們仍然是匿名訪客。
