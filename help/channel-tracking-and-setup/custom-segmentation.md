...
在使用此功能之前，請先瞭解您想要依據哪些接觸點資訊進行篩選。 請記得在CRM中為您的記錄型別使用完全相同的值。 此設定會從行銷funnel的頂部到底部篩選接觸點。
...
* 從左側邊欄的[!UICONTROL Segments]區段下的選項中選取[!UICONTROL Reporting]

步驟3 — 瞭解元件。

* 使用此圖例來瞭解此頁面上的各種圖示

![使用此圖例來瞭解此頁面上的各種圖示](assets/custom-segmentation-1.png)
...
![輸入區段名稱。 例如，您可以為](assets/custom-segmentation-2.png)使用一個區段

* 按一下加號圖示以顯示規則輸入欄位。 欄位挑選清單中的選項會直接從您的CRM中提取欄位

![按一下加號圖示以顯示規則輸入欄位。 選項](assets/custom-segmentation-3.png)
...
如果您是Marketo Measure Ultimate客戶，並將您的預設儀表板物件設定為連絡人，請勿使用下列兩個專屬於潛在客戶的欄位（[深入瞭解](/help/data-integrity-requirement.md){target="_blank"}）。
