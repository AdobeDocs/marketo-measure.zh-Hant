...
瞭解[設定UTM引數的最佳實務](/help/channel-tracking-and-setup/best-practices-for-setting-up-utm-parameters.md)。
...
* _[!DNL Marketo Measure]最佳實務&#x200B;:_請勿使用此欄位來呼叫子管道，否則，在實際管道上產生報告時可能會遇到困難。 用它來識別您的行銷工具或管道。 例如，如果您想要使用電子郵件來行銷您的產品，媒體就是電子郵件。
...
* _[!DNL Marketo Measure]最佳實務&#x200B;:_此引數會追蹤流量的來源，因此不適於用來指出廣告型別，例如重新鎖定目標、贊助等。 它最好用於追蹤較高層級的子管道。 請記住，您回答的問題為「我的流量來自何處？」 您在尋找反向連結。 在此範例中，UTM Source是廣告所在的位置（而非實際網頁，因為系統會自動在標籤外部追蹤該網頁）。 如果您追蹤的是滴答式電子郵件行銷活動，則滴答式電子郵件為來源。
...
* 此引數對應至SFDC中的[!DNL Marketo Measure] 「廣告行銷活動名稱欄位」。
