---
description: Marketo Measure使用者的自訂歸因模型與設定指南
title: 自訂歸因模型與設定
...
1. 決定您要在自訂模型中包含哪些階段。
   ...
   ![您最多可以有六個自訂階段（不包括預設值：](assets/custom-models-1.png)）
   ...
   ![在以下範例中，自訂「MQL」階段是使用](assets/custom-models-10.png)來定義

   >[!CAUTION]
   >
   >別忘了啟用自訂欄位的歷程記錄追蹤。

如果您的自訂模型中使用了自訂欄位，則必須在CRM中啟用欄位歷史記錄追蹤。 如需啟用欄位記錄追蹤的指示，請參閱[自訂模型設定：啟用欄位記錄追蹤](/help/channel-tracking-and-setup/custom-model-setup-enable-field-history-tracking.md)。
...
1. 決定自訂模型的歸因百分比。
   ...
   _自訂_&#x200B;欄的左側是&#x200B;**[!DNL Marketo Measure]機器學習模型**。 「機器學習」模型會根據贏得交易的相對重要性（取決於每個自訂階段所發生的情況），計算歸因加權。 如需機器學習模型的詳細資訊，請參閱[機器學習模型常見問題集](/help/channel-tracking-and-setup/machine-learning-model-faq.md)。

   ![自訂資料行的左側是Marketo Measure](assets/custom-models-2.png)

## 接觸點位置 {#touchpoint-positions}

在儲存及處理歸因百分比後，接觸點將會更新並接收其新階段和位置。 階段轉換之前最近發生的接觸點將獲得該階段的評分（如下所示）。 自訂權重和收入也會重新分配。

![在儲存及處理歸因百分比後，接觸點將會是](assets/custom-models-3.png)

## funnel階段和自訂模型階段之間的差異 {#the-difference-between-funnel-stages-and-custom-model-stages}

現在，即使您未啟用「自訂模型」，仍可在行銷Funnel中看到自訂階段。 這可透過使用Funnel Stage功能完成。 funnel階段現在可讓您將階段新增至funnel，但看不到其歸因。

funnel階段仍會以接觸點形式受到追蹤，且仍會顯示為您CRM中的接觸點位置。 如果沒有自訂模型，只要有表單填滿（10%為中間接觸），這些接觸點仍可能獲得中間接觸歸因，但若只是網頁造訪，則歸因點數為零。

如以下所示，我們將Dynamic階段納入Funnel階段中。 這表示我們有此位置包含「盡職調查」的接觸點，但若未啟用「自訂模型」（最多10%），這些接觸點將只會獲得中間接觸歸因評分。

![如以下所示，我們已將勤奮階段作為分開的階段納入](assets/custom-models-7.png)
