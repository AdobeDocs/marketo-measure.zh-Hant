---
description: Marketo Measure使用者的自訂Campaign同步指引
title: 自訂Campaign同步
...
1. 選擇接觸點日期。 在您輸入大括弧`{`後，可能的日期清單就會顯示 — 然後您可以選取要選擇套用至所有從規則建立的接觸點的日期。
   ...
   ![1. 選擇接觸點日期。 可能的日期清單將會顯示](assets/offline-channels-11.png)
...
1. 按一下核取記號，然後視需要為其他行銷活動新增其他規則。
   ...
   ![1. 按一下核取記號，然後將其他行銷活動的其他規則新增為](assets/offline-channels-12.png)

   >[!NOTE]
   >
   >現在規則已與CRM同步功能一起定義，接著所述的規則自然會開始衝突。 如果選擇繼續使用自訂Campaign同步處理&#x200B;_和_ CRM同步處理型別，請務必建立規則，以免忽略您的CRM同步處理型別。

   ![現在規則已與CRM Sync一起定義，因此](assets/offline-channels-13.png)
...
您可以在Campaign中輕鬆檢查購買者接觸點和Buyer Attribution Touchpoint記錄，確保規則正常運作。 以下是[!DNL Marketo Measure]使用適當的動態接觸點日期（從促銷活動提取）建立的BAT。 「建立日期」欄位位於下圖。

![您可以輕鬆檢查購買者接觸點和Buyer Attribution Touchpoint記錄](assets/offline-channels-14.png)

## 測試 {#testing}

1. Campaign同步功能隨附測試功能，可讓您檢查您建立的規則是否實際符合Campaign條件。 按一下[!UICONTROL Test]按鈕以開始。 必須先儲存規則，才能開始測試。

   ![1. Campaign同步功能隨附測試功能，因此](assets/offline-channels-15.jpg)

   畫面上會出現快顯視窗，您可於其中輸入行銷活動ID （CRM中的15或18個字元）以進行測試。 這主要是要輸入您嘗試同步之CRM的促銷活動ID，確保符合您建立的規則。

   ![會出現一個快顯視窗，您可以在其中輸入行銷活動ID （或](assets/offline-channels-16.png)）

1. 按一下[!UICONTROL Test]之後，您將會看到適用於接觸點的促銷活動名稱和促銷活動成員數目。 下表會顯示與您的行銷活動ID相符的所有規則。 只會顯示相符專案。

   ![1. 按一下[測試]後，您會看到](assets/offline-channels-17.png)的名稱

1. 您也可以按一下「成員」計數，以檢視屬於Campaign規則適用性一部分的「銷售機會」和「聯絡人」及其ID清單。 這只是一個範例集，會顯示最多50筆記錄，讓您瞭解哪些記錄符合條件。

   ![1. 您也可以按一下成員計數來檢視清單](assets/offline-channels-18.jpg)
