...
![當銷售機會或連絡人直接新增至行銷活動時，即會輸入](assets/dynamics-lists-1.png)
...
![若要在接觸點歷程中包含這些記錄，會有一些記錄](assets/dynamics-lists-10.png)

您可以選擇將具有行銷活動回應的所有記錄包含在行銷活動中，或僅包含具有「感興趣」回應的記錄，或者在預設情況下，您完全不能包含行銷活動回應。 您可以讓欄位保持空白或明確選擇排除它。

[!DNL Marketo Measure]不支援自訂回應值。

![Marketo Measure不支援自訂回應值。](assets/dynamics-lists-2.png)

這些是Campaign回應的庫存回應值：

![這些是行銷活動回應的庫存回應值：](assets/dynamics-lists-3.png)
...
![使用此欄位的一個常見範例是清單](assets/dynamics-lists-4.png)的事件
...
![Marketo Measure僅支援靜態行銷清單。 我們不支援](assets/dynamics-lists-5.png)
...
![若要啟用接觸點的行銷清單，有單獨的設定](assets/legacy-processes-10.png)
...
![行銷清單的接觸點日期通常是建立的ListMember](assets/dynamics-lists-6.png)
...
![對於每個行銷活動型別，它都可以對應到頻道，以及](assets/dynamics-lists-7.png)

![對於每個行銷活動型別，它都可以對應到頻道，以及](assets/dynamics-lists-8.png)
