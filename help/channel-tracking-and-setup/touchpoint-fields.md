...
![從Marketo Measure設定功能表，瀏覽至接觸點欄位](assets/touchpoint-fields-1.png)
...
![。&amp;42；告訴我們我們正在擷取完整的字串](assets/touchpoint-fields-10.png)
...
![請注意，您使用的是正斜線，而不是](assets/touchpoint-fields-11.png)
...
![{6}是我們正在擷取的字元數](assets/touchpoint-fields-12.png)
...
![{6}是我們正在擷取的字元數](assets/touchpoint-fields-13.png)
...
![您針對「夏季促銷活動結束」建立了行銷活動](assets/touchpoint-fields-2.png)
...
![若要擷取10，您可以將值定義為「BZ=\d{2}-\d{2}-\d{2}-\d{2}-(\d{2})」](assets/touchpoint-fields-3.png)
...
![Segmentc和Gradec的Opportunity物件上有個別的欄位](assets/touchpoint-fields-4.png)
...
![此](assets/touchpoint-fields-5.png)提供使用接觸點欄位建立區段的功能
...
![您的設定看起來會類似於下面的熒幕擷圖。 區段名稱](assets/touchpoint-fields-6.png)

規則會參考相同的接觸點欄位，並搜尋「不等於null」的值。

![規則會參考相同的接觸點欄位，並搜尋](assets/touchpoint-fields-7.png)的值
...
需要額外的工作才能公開[!DNL Marketo Measure] Data Warehouse結構描述中的新欄位。 目前，欄位會透過設定和設定公開，因此您可以在建立區段或建立接觸點刪除規則時使用接觸點欄位。
...
![有一個線上工具(https://regex101.com/(https://regex101.com/){target="blank"})，您可以執行並測試](assets/touchpoint-fields-8.png)

![有一個線上工具(https://regex101.com/(https://regex101.com/){target="blank"})，您可以執行並測試](assets/touchpoint-fields-9.png)
