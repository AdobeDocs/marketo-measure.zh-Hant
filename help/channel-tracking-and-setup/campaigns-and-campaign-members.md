...
![Marketo Measure Salesforce封裝將包含標示為](assets/dynamics-lists-1.png)的欄位

挑選清單中可用的選項包括：

![挑選清單中可用的選項為：](assets/dynamics-lists-10.png)
...
![如果您在使用Marketo Measure之前使用行銷自動化平台](assets/dynamics-lists-2.png)
...
![若啟用，Marketo Measure會將狀態值推送至](assets/dynamics-lists-3.png)
...
![如果清單是在](assets/dynamics-lists-4.png)之後上傳的日/周/月，則可能必須執行此操作
...
![大量更新接觸點日期包含在已安裝的Marketo中](assets/dynamics-lists-5.png)

如果需要更新大量行銷活動會員記錄，您可以使用[!UICONTROL Bulk Update Touchpoint Date]按鈕進行大量編輯。

如果此介面未涵蓋不重複的使用案例，您也可以使用[資料載入器](https://dataloader.io/){target="_blank"}來匯出記錄、進行變更，以及重新上傳記錄。
...
![有一個搜尋無法運作，它顯示在](assets/legacy-processes-10.png)中
...
![設定您的日期和時間後，按一下「更新選取的專案」](assets/dynamics-lists-6.png)

## 行銷活動成本 {#campaign-costs}

在本文[中瞭解所有促銷活動成本](/help/crm-campaign-costs.md){target="_blank"}。

## 行銷活動成員移除 {#campaign-member-removal}

[!DNL Marketo Measure]在Salesforce中追蹤任何已刪除記錄（無論已刪除潛在客戶、帳戶或商機）的方式，就是在API中檢視這些記錄並追蹤專案是否標示為「IsDeleted」。 不幸的是，對於行銷活動成員，Salesforce推出了從行銷活動中刪除這些行銷活動成員的不同方式，這些成員實際上會標籤為「已移除」而非「已刪除」，因此問題為接觸點仍然存在於Salesforce中，並且與已刪除的行銷活動成員相關。
