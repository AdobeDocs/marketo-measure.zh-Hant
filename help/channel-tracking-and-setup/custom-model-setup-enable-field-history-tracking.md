---
description: 自訂模型設定 — 為Marketo Measure使用者啟用欄位歷史記錄追蹤指引
...
如果您決定將自訂欄位加入為自訂歸因模型中的階段，則必須為此欄位啟用欄位歷程記錄追蹤&#x200B;****。 啟用欄位歷史記錄追蹤將允許[!DNL Salesforce]透過在「歷史記錄追蹤」表格中建立記錄，隨時追蹤編輯自訂欄位。[!DNL Marketo Measure] 可以下載該表格，並使用此資訊來測量「轉換」發生的時間和日期。 若沒有欄位歷史記錄追蹤，[!DNL Marketo Measure]將無法追蹤與此欄位相關的變更。
...
1. 前往自訂欄位所在的物件，然後按一下&#x200B;**[!UICONTROL Set History Tracking]**&#x200B;按鈕。
   ...
   ![1. 移至自訂欄位所在的物件，然後按一下](assets/custom-models-1.png)

1. 選取您要追蹤變更的欄位。

   ![1. 選取您要追蹤變更的欄位。](assets/custom-models-10.png)
