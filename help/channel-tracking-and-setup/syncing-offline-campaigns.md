---
description: 同步Marketo Measure使用者的離線行銷活動指引
title: 同步離線行銷活動
...
準確地追蹤離線行銷活動，並瞭解其與您數位行銷工作的比較情形，可能相當困難。[!DNL Marketo Measure] 可讓您在[!DNL Salesforce]中追蹤接觸點並將其歸因於離線行銷活動，即使在事件發生幾週後才會建立[!DNL Salesforce]行銷活動的情況下也是如此。
...
![您的上傳日期將會晚於會議日期三週。](assets/dynamics-lists-1.png)
...
![在啟用](assets/dynamics-lists-10.png)的接觸點之前，請務必更新接觸點日期
...
![若要在Salesforce中建立行銷活動，請導覽至「行銷活動」](assets/dynamics-lists-2.png)

建立此行銷活動時，按一下「[!UICONTROL Enable Buyer Touchpoints]」欄位，然後從選擇清單中選取下列其中一個選項：

![當您建立此行銷活動時，請按一下[啟用購買者接觸點]](assets/dynamics-lists-3.png)
