...
![從與下一個](assets/touchpoint-settings-10.png)相關的運運算元清單中選擇
...
![或者，在群組之外新增陳述式，以使用&quot;OR&quot;邏輯](assets/touchpoint-settings-11.png)

`3)`如果您的規則變得複雜，而且您需要重新建立群組並對每個陳述式進行微幅變更，請使用[!UICONTROL Clone]選項來簡化程式。

![3)如果規則變得複雜，您需要重新建立群組](assets/touchpoint-settings-6.png)

如果您犯錯，請不要擔心。 您也可以刪除陳述式的個別列或完整群組。

![如果發生錯誤，請不要擔心。 您可以刪除個別列](assets/touchpoint-settings-7.png)

`4)`如果您想要將購買者歸因接觸點套用至這兩個物件，請設定購買者歸因接觸點的規則。 我們的彈性可讓您為一個物件或兩個物件設定規則，也可以選擇為兩個物件設定規則（如果適用）。

![4)如果您想要的話，請設定購買者歸因接觸點的規則](assets/touchpoint-settings-8.png)
