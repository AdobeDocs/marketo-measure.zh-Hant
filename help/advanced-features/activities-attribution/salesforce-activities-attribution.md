...
![顯示歸因規則之活動設定頁面的「設定」標籤](assets/1.png)
...
![促銷活動名稱欄位具有動態取代引數範例，適用於傳出銷售](assets/2.png)
...
![活動規則組態介面顯示篩選條件及and/or陳述式選項](assets/3.png)
...
![Buyer Touchpoint日期欄位選取專案，顯示標準和自訂日期欄位選項](assets/4.png)

別忘了為任務或事件設定不同的規則。 您必須知道您的銷售團隊使用哪個物件來記錄其活動。

![物件型別選擇器，顯示活動規則的任務和事件選項](assets/5.png)
...
![行銷管道設定頁面顯示具有萬用字元運運算元選項的促銷活動對應](assets/6.png)
...
![行銷支出上傳介面，用於依月份輸入管道成本並進行ROI追蹤](assets/7.png)
