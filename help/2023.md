---
description: 2023年Marketo Measure使用者發行說明指引
title: 2023年發行說明
...
重新設計的[網頁流量儀表板](/help/web-traffic-dashboard.md){target="_blank"}現在可供所有客戶存取。 此儀表板提供網站訪客互動的完整總覽。 您可以分析量度，例如每個URL的不重複訪客計數、整體造訪、頁面檢視，以及來自特定表單URL或登入頁面的表單提交。 您也可以追蹤每月流量趨勢，並找出高績效的付費媒體，協助您調整策略以取得最佳收入。
...
[資料倉儲](/help/data-warehouse-schema.md){target="_blank"}中的URL表格現在除了包含網頁資料表格外，還會包含頁面標題欄位。
...
* [新增Discover儀表板指南](/help/new-discover-dashboard-guide.md){target="_blank"}
* [探索儀表板基本知識](/help/discover-dashboard-basics.md){target="_blank"}
* [收入總覽儀表板](/help/revenue-overview-dashboard.md){target="_blank"}
* [已歸因的收入儀表板](/help/attributed-revenue-dashboard.md){target="_blank"}
* [ROI控制面板](/help/roi-dashboard.md){target="_blank"}
* [Passport儀表板](/help/passport-dashboard.md){target="_blank"}
...
我們將逐步淘汰匯出至Lead/Contact物件的工作，以簡化整合，並免除匯出至Salesforce標準物件的需求。 下列非正規化欄位也會遭到取代，因為客戶可以從其接觸點物件取得相同的資料。_**淘汰的時間表為2024年6月。**_
...
![為具有或不具有接觸點的潛在客戶與聯絡人建立新的報告型別。](assets/release-notes-3.png)

* 建立可擷取任何現有報告（其使用已移除的欄位）功能的報告。 在此程式中，您將需要變更報表中的欄位，如下所示：

* 移除銷售機會/聯絡人FT/LC欄位：

![移除銷售機會/連絡人FT/LC欄位：](assets/release-notes-5.png)

* 新增接觸點欄位：

![新增接觸點欄位：](assets/release-notes-4.png)

* 「接觸點位置」篩選器以及任何使用FT/LC欄位（包括「日期」欄位）的篩選器，都應更新如下：

![接觸點位置篩選器以及任何使用FT/LC欄位的篩選器，](assets/release-notes-2.png)

![接觸點位置篩選器以及任何使用FT/LC欄位的篩選器，](assets/release-notes-1.png)
...
* 將棄用具有RefreshToken的過時方法OAuth。 請參閱[本指南](/help/oauth-with-azure-active-directory-for-dynamics-crm.md){target="_blank"}，瞭解如何更新您的認證，以遵循Microsoft使用ClientSecret的最佳實務。

#### 「custom_properties」欄位

在我們Data Warehouse中，「custom_properties」欄位一直是固定結構描述未涵蓋之其他資料點的儲存空間。 此欄位以JSON格式儲存，其使用方式有限，而且與SQL查詢的整合可能會很複雜，從而影響效能。 基於這些因素，我們決定棄用此欄位。 這項變更主要影響Azure表格儲存空間中的資料處理層，以及匯出至資料倉儲的資料。
...
[本文](/help/configuration-and-setup/salesforce-package-consolidation.md){target="_blank"}包含從舊版報告套件重新建立報告和儀表板的指南。
