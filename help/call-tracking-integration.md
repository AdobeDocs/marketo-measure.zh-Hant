---
description: Marketo Measure使用者的呼叫追蹤整合指引
title: 呼叫追蹤整合
...
一般意義上的![「通話追蹤」是來自此類公司的產品](assets/other-resources-6.png)
...
![透過整合，您可以看到網頁工作階段實際上是](assets/other-resources-4.png)

## 運作方式 {#how-it-works}

CallTrackingMetrics必須在其終端執行一些開發工作，才能正常運作。 透過CallTrackingMetrics放置於您網站上的JavaScript，可以從`_biz_uid` Cookie抓取[!DNL Marketo Measure]。 然後，CallTrackingMetrics會儲存此「BizibleId」。
...
![以下是BizibleId為](assets/other-resources-5.png)的工作記錄範例
...
![已從](assets/marketo-engage-activities-01.png)從任務提取接觸點型別「呼叫」
...
![Marketo Measure通常會推送的接觸點型別值為「網頁造訪」，](assets/other-resources-1.png)
...
請注意，如果有值，我們只會將所有網路工作階段視為30分鐘。 如果在12:17pm （網站上的工作階段開始）點選Google廣告，但直到1:05pm才發生電話呼叫，則我們不會合併網頁工作階段和電話呼叫。 [!DNL Marketo Measure]會建立個別的[!DNL Salesforce Task]接觸點來追蹤電話通話，但不會有任何Web工作階段資料。

![如果有值，請注意，我們只考慮所有網頁](assets/other-resources-2.png)
