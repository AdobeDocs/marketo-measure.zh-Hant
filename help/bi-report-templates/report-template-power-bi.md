...
開啟Adobe [!DNL Marketo Measure]報告範本Power BI檔案。

Marketo Measure範本的![Power BI連線對話方塊](assets/marketo-measure-report-template-power-bi-1.png)

您可以在[!DNL Marketo Measure]資訊頁上的[!DNL Data Warehouse] UI中找到您特定的伺服器、倉儲和結構描述資訊。 有關如何找到此頁面的說明，請參閱[這裡](/help/data-warehouse/data-warehouse-access-reader-account.md){target="_blank"}。
...
開啟檔案時輸入的引數可用來建構從資料倉儲匯入表格的原生查詢。 您仍需要設定與您的[!DNL Snowflake]執行個體的資料連線。 為此，您需要相同的伺服器和倉儲名稱以及您的使用者名稱和密碼。 如需在何處尋找使用者名稱及重設密碼的詳細資訊，請參閱[此處](/help/data-warehouse/data-warehouse-access-reader-account.md){target="_blank"}。
...
Power BI中的![原生查詢核准對話方塊](assets/marketo-measure-report-template-power-bi-2.png)
...
![查詢編輯器顯示日期篩選引數](assets/marketo-measure-report-template-power-bi-3.png)
...
![Power Query編輯器顯示套用的轉換步驟](assets/marketo-measure-report-template-power-bi-4.png)
...
>[!NOTE]
> 新增其他外部索引鍵值時請小心。 Power BI通常設定為自動偵測模型中的關係，而新增外部索引鍵值可能會導致表格之間出現不良連結和/或停用現有關係。
> [!DNL Marketo Measure]資料倉儲中的大多數資料表都包含非正規化的維度資料。 我們已儘量在Power BI中標準化及清理模型，以提高效能和資料準確度。 在事實表格中加入任何其他非正規化欄位時，請務必小心，這樣可能會破壞跨表格的維度篩選，並可能導致不準確的報告。

![資料行選擇對話方塊顯示已移除的資料行組態](assets/marketo-measure-report-template-power-bi-5.png)

### 已重新命名欄 {#renamed-columns}

已重新命名表格和欄，使其更方便使用，並標準化命名慣例。 若要檢視欄名稱變更，請導覽至任何表格中「已移除其他欄」步驟後面的「已重新命名欄」步驟。

![已重新命名顯示欄名稱對應的欄步驟](assets/marketo-measure-report-template-power-bi-6.png)
...
![Power Query中的區段名稱對應表](assets/marketo-measure-report-template-power-bi-7.png)
...
![顯示區分大小寫識別碼轉換的接觸點表格](assets/marketo-measure-report-template-power-bi-8.png)

![具有聯結ID資料行的Campaign資料表區分大小寫的比對](assets/marketo-measure-report-template-power-bi-9.png)
...
![具有公司轉換率資料行](assets/marketo-measure-report-template-power-bi-10.png)的機會資料表

儲存在[!DNL Snowflake]中的轉換率表格包含每個轉換的日期範圍。 Power BI不允許在計算上加入條件（即介於日期範圍之間）。 為了聯結日期，我們在「轉換率」表格中新增了步驟，以展開資料列，讓轉換日期範圍內的每個日期都有一列。

![具有展開日期列的轉換率表格](assets/marketo-measure-report-template-power-bi-11.png)

## 資料模型 {#data-model}

按一下下方影像以取得其完整大小版本。

[![顯示資料表關係的Power BI資料模型圖表](assets/marketo-measure-report-template-power-bi-12.png)](/help/bi-report-templates/assets/power-bi-data-model.png){target="_blank"}
...
![顯示DAX計算邏輯的貨幣轉換公式](assets/marketo-measure-report-template-power-bi-13.png)

## 資料定義 {#data-definitions}

已將表格、自訂欄和測量的定義新增至Power BI模型。

![顯示定義描述的資料表屬性](assets/marketo-measure-report-template-power-bi-14.png)

![資料模型](assets/marketo-measure-report-template-power-bi-15.png)中的自訂資料行定義

![含DAX公式的量值定義](assets/marketo-measure-report-template-power-bi-16.png)
