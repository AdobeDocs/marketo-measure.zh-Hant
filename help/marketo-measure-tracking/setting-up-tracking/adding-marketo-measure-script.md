...
新增 [!DNL Marketo Measure] 透過標籤管理解決方案編寫指令碼，您將需要建立新標籤並在其中新增JavaScript。 將此標籤套用至網站上您要追蹤的所有頁面。
...
不過，如果您使用任何個別或國際網域，請務必讓您的 [!DNL Marketo Measure] 諮詢顧問知道。 網域需要手動新增到您在上的帳戶 [!DNL Marketo Measure] 結束 [!DNL Marketo Measure] 會知道將其他網域的資料連結至您的帳戶。 因此，請將任何個別/國際網域傳送至 [!DNL Marketo Measure] 顧問。
...
您有任何不安全的頁面嗎？ 若是如此，您會想要保護安全性，因為導覽安全/不安全的頁面時會中斷追蹤工作階段。
...
* 解決方案：您將需要強制其中一個表單向報告使用者 [!DNL Marketo Measure] 根據快取資料並討論放棄作法。 一般而言， [報表使用者代碼](/help/marketo-measure-tracking/setting-up-tracking/adding-marketo-measure-script-to-different-form-providers/ajax-form-handling.md){target="_blank"} 可以解決這個問題。
