...
![開啟Adobe Marketo Measure報表範本Power BI檔案。](assets/marketo-bi-1.png)

您可以在[!DNL Marketo Measure]資訊頁上的[!DNL Data Warehouse] UI中找到您特定的伺服器、倉儲和結構描述資訊。 有關如何找到此頁面的說明，請參閱[這裡](/help/data-warehouse-access-reader-account.md){target="_blank"}。
...
開啟檔案時輸入的引數可用來建構從資料倉儲匯入表格的原生查詢。 您仍需要設定與您的[!DNL Snowflake]執行個體的資料連線。 為此，您需要相同的伺服器和倉儲名稱以及您的使用者名稱和密碼。 如需在何處尋找使用者名稱及重設密碼的詳細資訊，請參閱[此處](/help/data-warehouse-access-reader-account.md){target="_blank"}。
...
![若要限制匯入模型的資料，每個資料表都會設定](assets/marketo-bi-2.png)
...
![因為日期篩選器已套用至](assets/marketo-bi-3.png)的修改日期
...
![已經將一些轉換套用到Power Query中的資料。](assets/marketo-bi-6.png)
...
![Marketo Measure Data Warehouse中的大多數資料表都包含非正規化的維度](assets/marketo-bi-7.png)

### 已重新命名欄 {#renamed-columns}

已重新命名表格和欄，使其更方便使用，並標準化命名慣例。 若要檢視欄名稱變更，請導覽至任何表格中「已移除其他欄」步驟後面的「已重新命名欄」步驟。

![已重新命名資料表和資料行，使其更方便使用，](assets/marketo-bi-5.png)
...
![由於區段名稱是可自訂的，因此它們在](assets/marketo-bi-4.png)中具有一般資料行名稱
...
![/貼上函式並篩選。](assets/marketo-bi-8.png)

![/貼上函式並篩選。](assets/marketo-bi-11.png)
...
![若要將貨幣轉換功能新增至模型中的計算，我們已](assets/marketo-bi-10.png)

儲存在[!DNL Snowflake]中的轉換率表格包含每個轉換的日期範圍。 Power BI不允許在計算上加入條件（即介於日期範圍之間）。 為了聯結日期，我們在「轉換率」表格中新增了步驟，以展開資料列，讓轉換日期範圍內的每個日期都有一列。

![儲存在Snowflake中的轉換率表格包含日期範圍](assets/marketo-bi-9.png)

## 資料模型 {#data-model}

按一下下方影像以取得其完整大小版本。

[![按一下下方影像以取得其完整大小版本。](assets/marketo-bi-12.png)](assets/power-model-1.png){target="_blank"}
...
![此模型中的貨幣轉換量值會取代1.0](assets/marketo-bi-13.png)的值

## 資料定義 {#data-definitions}

已將表格、自訂欄和測量的定義新增至Power BI模型。

![資料表的定義已新增至Power BI模型，自訂](assets/marketo-bi-15.png)

![資料表的定義已新增至Power BI模型，自訂](assets/marketo-bi-16.png)

![資料表的定義已新增至Power BI模型，自訂](assets/marketo-bi-14.png)
