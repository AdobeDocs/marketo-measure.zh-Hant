---
description: Marketo Measure使用者的單一登入指引
title: 單一登入
...
>[!CAUTION]
>
>本文概述單一登入(SSO)和進階CRM使用者管理。 如果您的帳戶是在2020年9月10日&#x200B;**之後布建**，請忽略本文，因為SSO和Identity Management將會在[Adobe Admin Console中設定為您的 [!DNL Marketo Measure] 整合](/help/implementation-guide.md)。
...
![若要開始，請瀏覽至](assets/compliance-resources-1.png)中的[設定安全性驗證]頁面
...
填寫身分提供者設定的連線設定
    ...
    a。 您的身分提供者可能會提供IdP中繼資料.xml檔案，以提取必要的設定欄位。 請載入.xml檔案的內容，或從身份提供者組態程式期間取得的輸出填寫以下三個欄位。 **您不需要同時完成兩項。**
...
設定您的使用者屬性設定，並將其對應至個別的名字、姓氏和電子郵件地址。
    ...
    ii. 名字屬性：提供識別提供者用於使用者名字的屬性名稱。
    iii。 姓氏屬性：提供識別提供者用於使用者姓氏的屬性名稱。
    
    b。 提示：如果您現在測試您的SAML設定，我們會剖析您可用於此節的電子郵件、名字和姓氏屬性。

![b。 提示：如果您現在測試您的SAML設定，我們將剖析](assets/discover-control-1.png)

設定您的使用者角色設定，並將其對應至分類自IdP的個別角色或群組。

    a。 客戶可選擇根據身分提供者中定義的群組來指派 [!DNL Marketo Measure] 使用者角色。 透過輸入SAML屬性， [!DNL Marketo Measure] 將能夠將您的使用者角色和群組對應到 [!DNL Marketo Measure] 使用者許可權。 我們強烈建議您設定這些角色，讓您的 [!DNL Marketo Measure] 管理員擁有足夠的許可權來更新您的帳戶。
    
    b. 如果未對應任何角色或群組，則預設設定為身分提供者中的所有員工都將擁有標準使用者存取權。
    
    i. [!DNL Marketo Measure] 標準使用者：為應該擁有 [!DNL Marketo Measure] 應用程式的唯讀存取權的使用者，提供角色或群組值（來自您的SSO提供者）。
    ii. [!DNL Marketo Measure] 帳戶管理員使用者：為應該擁有 [!DNL Marketo Measure] 應用程式管理存取權的使用者，提供角色或群組值（來自您的SSO提供者）。 這表示該角色有權變更與您的帳戶相關的組態和設定。
    iii. 您必須在IdP中擁有屬性，而且其名稱為「群組」，用來存放您放入「Bizible Standard User」或「Bizible Account Admin User」屬性的值。
    
    c. 如果多個角色或群組應該對應至一個角色，請輸入每個值（以逗號分隔）。

![c。 如果多個角色或群組應該對應到一個角色，](assets/discover-control-2.png)

測試單一登入設定

    a。 按一下[儲存]之前，您必須先按一下[!UICONTROL Test SAML Authentication]按鈕，確認您的設定已正確設定。
    
    b。 如果您看到「失敗」錯誤，請依照訊息操作，然後再試一次。

![b。 如果您看到「失敗」錯誤，請依照訊息操作，並嘗試](assets/discover-control-3.png)

儲存您的設定，並指示您的同事搭配您新的自訂登入URL使用[!UICONTROL Single Sign On]。

    a。 重要：儲存新的驗證設定後，工作階段可能會在您導覽至新頁面後結束，因為您已停用CRM使用者的登入功能並啟用自訂SSO。

![a。 重要：儲存新的驗證設定後，就可以](assets/discover-control-3.png)
...
![c。 恭喜！ 您已成功設定](assets/discover-control-3.png)的單一登入
...
![若為Salesforce角色，請使用每個角色的名稱。 所有角色](assets/discover-control-3.png)

**動態角色**

對於[!DNL Dynamics]角色，請使用每個安全性角色的名稱。 您可以在[!UICONTROL Settings] >[!UICONTROL Security] >[!UICONTROL Security Roles]功能表找到所有安全性角色。

![對於Dynamics角色，請使用每個安全性角色的名稱。 全部](assets/discover-control-3.png)
...
![一旦設定自訂SSO，使用者頁面就會是](assets/discover-control-3.png)
