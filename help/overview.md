...
[!DNL Marketo Measure] （前身為Bizible）為行銷人員提供insight，讓行銷人員能夠最有效率地推動公司營收並最大化投資報酬。[!DNL Marketo Measure] 是一種行銷歸因解決方案，可自動追蹤和報告管道績效，讓您可見哪些管道可推動最多客戶參與，並據此將行銷支出最佳化。
...
![Marketo Measure層級1-3](assets/marketo-overview-1.png)
...
![行銷人員應隨身攜帶其B2B資料(例如，帳戶、機會、](assets/marketo-overview-2.png)

**預設貨幣設定**

[!DNL Marketo Measure Ultimate]將預設貨幣設定為USD，直到使用者變更為止。 設定新的預設貨幣會更新資料，而不會重新處理。 只要選取的貨幣以目標ISO代碼的形式存在，就不需要提交轉換率。

![Marketo Measure Ultimate將預設貨幣設定為USD，直到](assets/marketo-overview-7.png)
...
來自指定資料之指定實體（例如，帳戶）的![資料](assets/marketo-overview-6.png)
...
![沒有針對funnel、迴旋鏢vs自訂模型的選擇。 所有階段皆為](assets/marketo-overview-4.png)

Campaign成員接觸點規則和活動接觸點規則是資料集專屬規則。

![促銷活動會員接觸點規則和活動接觸點規則是資料集專屬規則。](assets/marketo-overview-5.png)

![促銷活動會員接觸點規則和活動接觸點規則是資料集專屬規則。](assets/marketo-overview-8.png)
