...
如果您選擇在[!DNL Marketo Measure]沙箱環境中測試[!DNL Salesforce]，請依照這些指示，在您準備好後移轉至生產。 下列指示假設您已將[!DNL Marketo Measure]套件下載至您的沙箱組織、執行了必要的測試並準備將[!DNL Marketo Measure]推送至生產環境。

## 步驟1：將[!DNL Marketo Measure]套件安裝到您的生產[!DNL Salesforce]執行個體

* 使用&quot;[!DNL Marketo Measure]&quot;設定將[!UICONTROL All Users]套件安裝至生產環境

   * [基底封裝](https://appexchange.salesforce.com/appxListingDetail?listingId=a0N3000000B3KLuEAN){target="_blank"}

* 有關與[!DNL Marketo Measure]的[!DNL Salesforce]關係的詳細資訊，請參閱[本文章](/help/configuration-and-setup/how-marketo-measure-and-salesforce-interact.md)
...
* 系統會提示您確認刪除。 請務必仔細閱讀提示，並瞭解刪除的後果
  ...
  ![系統會提示您確認刪除。 確定讀取超過](assets/salesforce-migration-1.png)
...
* 在「[!UICONTROL Select CRM Connection]」強制回應對話方塊中，按一下[!UICONTROL Connect]平台旁的「[!DNL Salesforce]」動作，選取「[!UICONTROL Production]」選項
* 系統會提示您輸入憑證，請務必輸入生產登入詳細資訊

## 步驟4： Salesforce設定 {#salesforce-configuration}

[頁面配置](/help/configuration-and-setup/page-layout-instructions.md)
...
[自訂工作流程（如適用）](/help/channel-tracking-and-setup/using-a-custom-revenue-amount-field.md)
