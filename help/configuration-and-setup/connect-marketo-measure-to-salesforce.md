...
1. 在左側的設定選項欄中，按一下&#x200B;**[!UICONTROL Connections]**&#x200B;區段下的[!UICONTROL Integrations]。
   ...
   ![1. 在左側設定選項的欄中，按一下](assets/bizible-full-1.png)

1. 在[連線]中的CRM區段下，按一下&#x200B;**[!UICONTROL Set Up New CRM Connection]**。

   ![1. 在[連線]中的CRM區段下，按一下[設定新增]](assets/bizible-taxonomy-1.png)

1. 系統會顯示一個快顯視窗，要求您選取CRM連線。 按一下&#x200B;**[!UICONTROL Connect]**&#x200B;標誌旁的[!DNL Salesforce]。

   ![1. 系統會顯示一個快顯視窗，要求您選取CRM連線。 按一下](assets/connect-salesforce-1.png)
...
Marketo Measure採用CRM整合工作，透過整合的使用者與客戶的Salesforce介面。 透過此使用者進行的所有資料交換都使用Salesforce API積分。 您有能力將評分配額分配給整合使用者，這會用來規管過度的API呼叫。 此配額或限制每24小時重設一次。

您可以在Marketo Measure中透過&#x200B;**我的帳戶** > **設定** > **CRM** > **一般** > **每日CRM API限制**&#x200B;存取此限制，也可以為租使用者設定。

![您可以在Marketo Measure中透過以下方式存取此限制： 「我的帳戶設定」](assets/connect-salesforce-2.png)
...
1. 按一下「鎖定」圖示以進行編輯。
   ...
   ![1. 按一下「鎖定」圖示以進行編輯。](assets/connect-salesforce-3.png)

1. 輸入等於或大於100,000的所需限制。 完成時，按一下&#x200B;**儲存**。

   ![1. 輸入等於或大於100,000的所需限制。 按一下](assets/connect-salesforce-1.png)
