...
1. 若要開始設定Marketo活動，請瀏覽至&#x200B;**我的帳戶** > **設定** > **活動**。
   ...
   ![1. 若要開始設定Marketo活動，請瀏覽至「我的帳戶設定」](assets/marketo-engage-activities-01.png)

   ![1. 若要開始設定Marketo活動，請瀏覽至「我的帳戶設定」](assets/marketo-engage-activities-02.png)
...
1. 按一下[!UICONTROL Select Activities Types]下方的下拉式功能表，開始選擇各種型別。
   ...
   ![1. 按一下「選取活動型別」下的下拉式功能表以開始](assets/marketo-engage-activities-03.png)

1. 選取所有您需要的活動後，您會看到它們已填入您的[!UICONTROL Selected Activities List]和[!UICONTROL Define Rules]下。

   ![1. 選取所有您需要的活動後，您會看到](assets/marketo-engage-activities-04.png)
...
1. 首先，視活動型別而定，您可能需要設定[!DNL Marketo Measure]行銷活動名稱，以便稍後用於管道對應。 [!DNL Marketo Measure]個行銷活動名稱可在多個規則中重複使用。 這有助於在單一管道規則中使用更廣泛的名稱。 並非所有活動型別都包含Marketo程式，因此第一個步驟需要名稱。
   ...
   ![以下是這個額外步驟的範例：](assets/marketo-engage-activities-05.png)

1. 在「變更分數」範例中，由於促銷活動名稱是從Marketo方案提取的，因此需要輸入該資訊。 現在建立規則運算式。 在此範例之後，選取運運算元為&quot;[!UICONTROL New Value]&quot;且值為90的欄位&quot;[!UICONTROL is greater than]&quot;。

   您可以展開規則，並新增「and」或「or」陳述式來縮小結果範圍，藉此新增其他篩選器或條件。

   ![您可以展開規則並新增其他篩選器或條件](assets/marketo-engage-activities-06.png)

   ![您可以展開規則並新增其他篩選器或條件](assets/marketo-engage-activities-07.png)

1. 最後，選擇要用來作為接觸點日期的專案。 所有可用的日期或日期/時間欄位都會從Marketo顯示在這裡。 除非您有自訂日期欄位，否則您會看到&quot;[!UICONTROL Activity Date]&quot;。

   ![1. 最後，選擇要用來作為接觸點日期的專案。 所有可用](assets/marketo-engage-activities-08.png)

1. 請務必在此過程中按一下&#x200B;**[!UICONTROL Save As Draft]**，以免變更遺失。

   ![1. 請務必沿途按一下「另存為草稿」](assets/marketo-engage-activities-09.png)

1. 導覽至&#x200B;**[!UICONTROL Attribute Mapping]**&#x200B;標籤。

   ![1. 瀏覽至[屬性對應]索引標籤。](assets/marketo-engage-activities-10.png)
...
若為具有方案ID的活動型別中的任何規則，則Marketo方案頻道由方案決定。 我們使用方案頻道來對應到您的自訂離線頻道，因此您需要確定您的頻道已依照此處[的指示正確設定](/help/marketo-engage-programs-integration.md)。

對於沒有方案ID的活動型別中的任何規則，您的第一步是建立行銷活動名稱。 使用此行銷活動名稱來設定您在此佈局的自訂線上頻道[&#128279;](/help/channel-tracking-and-setup/online-custom-channel-setup.md)。
...
Mark按一下Facebook廣告並登陸wayneenterprises.com，在那裡他取得[!DNL Marketo Measure] ID為123且[!DNL Marketo Munchkin Id] 456的Cookie。 沒有表單填寫。

Wayne企業行銷團隊會傳送電子郵件爆炸訊息給特定目標銷售機會，其中一個是`mark@email.com`。

`mark@email.com`會收到電子郵件，然後點進並登陸`wayneenterprises.com`。 這會變成使用相同Cookie Id對`mark@email.com's`進行`wayneenterprise.com`次的第二次造訪，但沒有任何表單填入，因此對於[!DNL Marketo Measure]，他們仍然是匿名訪客。

Wayne Enterprises行銷團隊會建立Marketo活動規則，以產生「按一下電子郵件」活動型別的接觸點。

今天的實作將會從「按一下電子郵件」活動型別中的Marketo活動，為`mark@email.com`建立單一FT和LC接觸點。

透過這個Cookie對應增強功能，FT可以回到Facebook廣告獲得點數，而LC則可以點數到電子郵件。
