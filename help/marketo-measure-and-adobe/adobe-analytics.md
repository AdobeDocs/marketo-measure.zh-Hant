...
B2B客戶屬性整合可讓[!DNL Marketo Measure]和Adobe Analytics的共同使用者透過其與CRM （[!DNL Adobe Analytics]和[!DNL Marketo Measure]）的同步功能，利用衍生自[!DNL Microsoft Dynamics]歸因引擎的寶貴中繼資料，擴充其[!DNL Salesforce]使用者設定檔。 所有使用[!DNL Adobe Analytics]和[!DNL Marketo Measure]的客戶都可以免費使用。
...
1. 建立客戶屬性資料Source後，請導覽至「**[!UICONTROL Integrations]**」管理員功能表中的「**[!UICONTROL Connections]** > [!DNL Marketo Measure]」畫面，繼續設定程式。
...
1. 按一下&#x200B;**[!UICONTROL Set Up New Customer Attributes Connection]**&#x200B;按鈕，然後依照指示設定客戶屬性整合。 UI會提示您輸入您在核心服務主控台中建立客戶屬性Source時取得的別名ID和FTP連線資訊。 選取您要同步至您[!DNL Adobe Analytics]帳戶的帳戶屬性集。
   ...
   輸入您的Adobe IMS組織ID。 此ID會顯示在Adobe Experience Cloud Admin Console的右下角。 如需尋找此ID的詳細協助，請洽詢Adobe客戶團隊（您的客戶經理）。
...
1. 最後一個步驟是設定您要在其中使用屬性之Experience Cloud應用程式的屬性訂閱。 您可以設定[!DNL Adobe Analytics]或[!DNL Adobe Target]的訂閱。  如需如何執行此動作的詳細資訊[，請前往這裡](https://experienceleague.adobe.com/docs/core-services/interface/services/customer-attributes/subscription.html?lang=zh-Hant)。
...
<table>
 ...
 <tbody>
  ...
  <tr>
   <td>funnel最深層</td>
   <td>與指定使用者相關聯之任何開啟機會的最深funnel階段。</td>
  </tr>
  <tr>
   <td>開啟機會</td>
...
1. Content Analytics：使用收入量度來分析哪些網站內容最能吸引最終購買您的產品或服務的客戶，或觸及感興趣的特定funnel階段的客戶。
1. 即時交易支援：分析與CRM中特定開放機會相關聯之使用者的網站行為，以可操作的insight支援您的銷售團隊。
